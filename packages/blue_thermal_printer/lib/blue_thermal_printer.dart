...
class BlueThermalPrinter {
  ...
  Future<void> printCustom(
    ...
    // Determine if we should use bold based on size or resolved index
    bool isBold = resolvedIndex >= 1;
    if (size is num && size > 12) isBold = true;
    if (size is String) {
      final double? s = double.tryParse(size.replaceAll(RegExp(r'[^0-9.]'), ''));
      if (s != null && s > 12) isBold = true;
    }
  ...
  Future<void> printImage(String path) async {
    ...
    int xOffset = ((fullWidth - imgWidth) ~/ 2) + 0;
